                        ...
                        ->schema([
                            Forms\Components\Select::make('workshop_id')
                                ...
                                ->afterStateUpdated(fn ($state, $set) => 
                                    $set('price', optional(Workshop::find($state))->price ?? 0)
                                )
                                ->afterStateHydrated(fn ($state, $get, $set) => 
                                    $set('price', optional(Workshop::find($state))->price ?? 0)
                                ),
                            ...
                            Forms\Components\TextInput::make('quantity')
                                ...
                                ->afterStateUpdated(fn ($state, $get, $set) => 
                                    $set('total_amount', ($subTotal = $get('price') * $state) + $subTotal * 0.11)
                                    && $set('participants', array_pad($get('participants') ?? [], $state, ['name' => '', 'occupation' => '', 'email' => '']))
                                )
                                ->afterStateHydrated(fn ($state, $get, $set) => 
                                    $set('total_amount', ($subTotal = $get('price') * $state) + $subTotal * 0.11)
                                ),
